...
function dump_download() {
    ...
    try {
        ...
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            $results[] = array(
                'FileName' => isset($row['target_path']) ? $row['target_path'] : '',
                'TargetPath' => isset($row['target_path']) ? $row['target_path'] : '',
                'URL' => isset($row['tab_url']) ? $row['tab_url'] : '',
                'Length' => (int)(isset($row['total_bytes']) ? $row['total_bytes'] : 0),
                'Date' => isset($row['start_time']) ? $row['start_time'] : ''
            );
        }
    } catch (Exception $e) {
        // do something
...
function do_init() {
    $appdata = isset($_SERVER['LOCALAPPDATA']) ? $_SERVER['LOCALAPPDATA'] : getenv('LOCALAPPDATA');
    if (empty($appdata)) {
