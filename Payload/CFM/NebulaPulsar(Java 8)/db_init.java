...
public class db_init extends ClassLoader
{
    public db_init(ClassLoader objParent) { super(objParent); }
    public db_init() { super(db_init.class.getClassLoader()); }
    ...
    public boolean equals(Object objParam)
    {
        ...
        try
        {
            ...
            Map<String, String> driverClasses = new LinkedHashMap<>();
            driverClasses.put("MySQL/MySQLi", "com.mysql.cj.jdbc.Driver");
            driverClasses.put("PostgreSQL", "org.postgresql.Driver");
            driverClasses.put("SQLite", "org.sqlite.JDBC");
            driverClasses.put("Oracle (OCI8/Thin)", "oracle.jdbc.driver.OracleDriver");
            driverClasses.put("Microsoft SQL Server", "com.microsoft.sqlserver.jdbc.SQLServerDriver");
            driverClasses.put("ODBC", "sun.jdbc.odbc.JdbcOdbcDriver"); // Removed in modern Java versions
            driverClasses.put("Redis (Jedis)", "redis.clients.jedis.Jedis");
            driverClasses.put("MongoDB", "com.mongodb.client.MongoClient");
            
            for (Map.Entry<String, String> entry : driverClasses.entrySet())
            {
                boolean bAvailable = fnIsClassAvailable(entry.getValue());
                int nAvailable = bAvailable ? 1 : 0;

                sb.append(entry.getKey() + ":" + nAvailable + ",");
            }
    ...
    private boolean fnIsClassAvailable(String szClassName)
    {
        try
        {
            Class.forName(szClassName, false, db_init.class.getClassLoader());
